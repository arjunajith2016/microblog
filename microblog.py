...
@app.route('/')
def home():
    cur = g.db.execute('select title, text, date, time, id, user  from entries order by id desc')
    entries = [dict(title=row[0], text=row[1], date=row[2], time=row[3], id=row[4], user=row[5]) for row in cur.fetchall()]
...
def add_entry():
    ...
    if request.form['title'] =='':
        ...
        g.db.execute('insert into entries (title, text, date, time, user) values (?, ?, ?, ?, ?)', [request.form['title'], request.form['text'], current_date, current_time, session['user']])
        g.db.commit()
